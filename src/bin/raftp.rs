use anyhow::Result;
use clap::Parser;
use rathernet::raftp::AftpEditor;
use thiserror::Error;
...
fn parse_url(url: &str) -> Result<(String, String)> {
    ...
    let Some(user) = split.next() else {
        return Err(RaftpError::InvalidUrl(url.into()).into());
    };
    ...
    Ok((user.into(), format!("{}:{}", host, port)))
}

#[tokio::main]
async fn main() -> Result<()> {
    let cli = RaftpCli::parse();
    let (user, host) = parse_url(&cli.url)?;
    let mut editor = AftpEditor::try_new(&user, &host).await?;
    editor.login().await?;
